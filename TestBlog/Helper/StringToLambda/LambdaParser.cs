...
namespace StringToLambda
{
    ...
    public static class LambdaParser
    {
        ...
        public static Expression<Func<T, bool>> Parse<T>(string source, params string[] ns)
        {
            var parseResult = SymbolParser.Build(source);
            var parser = new ExpressionParserCore<T>(parseResult, typeof(Func<T, bool>), ns, null);
        ...
        /// 分析委托.
        /// </returns>
        public static Func<T, bool> Compile<T>(string source, Assembly[] assemblies, params string[] ns)
        {
            var parseResult = SymbolParser.Build(source);
            var parser = new ExpressionParserCore<T>(parseResult, typeof(T), ns, assemblies);
